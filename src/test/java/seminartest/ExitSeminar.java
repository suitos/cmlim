 ...
 * 9.세미나 시작
 * 10.세미나 종료
 * 11.발표자 종료 UI 확인(RST-530)
 * 12.게시자 종료 UI 혹인(RST-500,RST-501,RST-502,RST-503)
 * 13.운영자 종료 UI 확인(RST-531)
 * 14.마스터 종료 UI 확인
 * 15.로그인 참석자 종료 UI 확인(RST-513,RST-505,RST-547)
 * 16.로그인X 참석자 종료 UI 확인(RST-522,RST-523,RST-549)
 */
...
public class ExitSeminar {
	...
	@Test(priority=3)
	  public void EnterSeminar_Publisher() throws Exception{
		...
		JavascriptExecutor js = (JavascriptExecutor) Publisher_driver;
		js.executeScript("arguments[0].scrollIntoView();", Publisher_driver.findElement(By.xpath(CommonValues.XPATH_SEMINARVIEW_ENTER)));
	...
	@Test(priority=4)
	  public void EnterSeminar_Present() throws Exception{
		...
		try {
			...
			login_present.until(ExpectedConditions.textToBePresentInElement(Present_driver.findElement(By.xpath("//div[@id='profile-drop-down']")), Presenters.get(0).replace("@gmail.com", "") ));
	...
	@Test(priority=5)
	  public void EnterSeminar_Organizer() throws Exception{
		...
		try {
			...
			login_present.until(ExpectedConditions.textToBePresentInElement(Organizer_driver.findElement(By.xpath("//div[@id='profile-drop-down']")), Organizers.get(0).replace("@gmail.com", "") ));
		...
		js.executeScript("arguments[0].scrollIntoView();", Organizer_driver.findElement(By.xpath(CommonValues.XPATH_SEMINARVIEW_ENTER)));
	...
	@Test(priority=9)
	  public void StartSeminar() throws Exception{
		...
		WebElement StartSeminarSettingBtn = Present_driver
				.findElement(By.xpath("//button[@class='btn btn-primary btn-m']"));
		...
		WebElement StartSeminarBtn = Present_driver
				.findElement(By.xpath("//button[@class='btn btn-primary btn-xl seminar-start']"));
